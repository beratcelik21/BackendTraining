	...
	db.coll.find({ 'age': { $eq: 15 } }) // == : equal //? olanlar
	db.coll.find({ 'age': { $ne: 15 } }) // <> : not equal //?olmayanlar
	db.coll.find({ 'age': { $gt: 15 } }) // > : greather than //?buyuk olanlar
	db.coll.find({ 'age': { $gte: 15 } }) // >= : greather than equal  //?buyuk ve esit olanlar
	db.coll.find({ 'age': { $lt: 15 } }) // <= : less than equal//? kucuk olanlar
	db.coll.find({ 'age': { $lte: 15 } }) // <= : less than equal //?kucuk esit olanlar
	db.coll.find({ 'age': { $in: [10, 11, 12] } }) // in list //?belirtilen ozellik olanlar
	db.coll.find({ 'age': { $nin: [10, 11, 12] } }) // not in list //?belirtilenin dısında olanlar
	// Regex:
	// mongodb.com/docs/manual/reference/operator/query/regex/
	db.coll.find({ firstName: { $regex: 'Test' } }) // Contains 'Test'//? icerisinde test olanlar
	db.coll.find({ firstName: /Test/ }) // Contains 'Test' //? icerisinde test olanlar
	db.coll.find({ firstName: /test/i }) // Case-InSensitive  //? kucuk harf buyuk harf duyarsız
	db.coll.find({ firstName: /^Test/ }) // StartsWith 'Test'
	db.coll.find({ firstName: /Test$/ }) // EndsWith 'Test'
	// Logical:
	db.coll.find({ 'age': { $not: { $eq: 15 } } }) // NOT {EQUAL}  //?Age 15 e esit degilmi esit olmayanlar
	...
	db.coll.find().skip(5).limit(5) // LIMIT 5, 5 //? ilk be kaydı atlaikinci 5 kaydı getir.
	// Sort (1:ASC, -1:DESC):
	...
	// Count:
	db.coll.find().count()
	db.coll.countDocuments() // ShortHand find().count()//? var olan kayıt sayısı
	db.coll.countDocuments({ firstName: 'Test' })//? ismi test olanlar kactane
	db.coll.estimatedDocumentCount() // for bigData //? tahmini kayıt sayısı
	...
	db.coll.updateOne({ age: 19 }, { $set: { age: 199 } }) // Add/Update field//? Kayıt guncelleme
	db.coll.updateOne({ age: 19 }, { $set: { new_fields: 'new_value' } }) // Add/Update field //? Kayıt guncelleme
	db.coll.updateOne({ age: 19 }, { $set: { new_fields: 'new_value2' } }) // Add/Update field//? Kayıt guncelleme
